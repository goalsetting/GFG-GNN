...
def get_data(name = None, B_name= None):
    ...
    if name is None:
        if B_name is None:
            ...
            train_data = pd.read_csv(root_path+f'/dataset/{B_name}/train.txt', names=['u', 'i', 'l'], delimiter='\t', dtype=int)
            test_data = pd.read_csv(root_path+f'/dataset/{B_name}/test.txt', names=['u', 'i','l'], delimiter='\t', dtype=int)
    else:
        train_data = pd.read_csv(root_path+f'/dataset/{name}/train.txt', names=['u', 'i', 'l'], delimiter='\t', dtype=int)
        test_data = pd.read_csv(root_path+f'/dataset/{name}/test.txt', names=['u', 'i','l'], delimiter='\t', dtype=int)
    ...
    if name is None:
        if B_name is None:
            ...
            dataset.u_x = torch.FloatTensor(np.load(root_path + f'/dataset/{B_name}/bdt_u_features.npy'))
            dataset.i_x = torch.FloatTensor(np.load(root_path + f'/dataset/{B_name}/bdt_i_features.npy'))

    else:
        dataset.u_x = torch.FloatTensor(np.load(root_path+f'/dataset/{name}/{name}_u_features.npy'))
        dataset.i_x = torch.FloatTensor(np.load(root_path+f'/dataset/{name}/{name}_i_features.npy'))
...
def get_full(dataset, device):
    ...
    dataset.train_edge_x = torch.cat((dataset.u_x[dataset.train_edge[:, 0]],
                                      dataset.i_x[dataset.train_edge[:, 1]]), dim=1).to(device)

    dataset.test_edge_x = torch.cat((dataset.u_x[dataset.test_edge[:, 0]],
                                     dataset.i_x[dataset.test_edge[:, 1]]), dim=1).to(device)
...
def get_abcore_data(device, name = None, B_name = None):
    dataset = get_data(name,B_name)

    dataset = get_abcore(dataset, device)

    return dataset

def get_ori_data(device, name = None):
    dataset = get_data(name)
    dataset = get_full(dataset, device)
    return dataset

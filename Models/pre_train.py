...
class LoadDataset(Dataset):
    def __init__(self, data):
        self.x = data.u_x[data.train_u].cuda()
...
def pretrain_ae(model, x, n_clusters, epochs,name=None, root_path = None):
    ...
    if name is not None:
        torch.save(model.state_dict(), root_path + f'/model/ae_pre_train_{name}.pkl')
    else:
...
def pre_train(dataset, n_clusters, n_input, n_z, n_enc, hidden, pre_ae_epoch,name=None, root_path = None):
    setup_seed(10)
    model = AE(
        n_enc=n_enc,
        hidden=hidden,
    ...
    pretrain_ae(model, dataset.u_x[dataset.train_u], n_clusters, pre_ae_epoch,name,root_path)
